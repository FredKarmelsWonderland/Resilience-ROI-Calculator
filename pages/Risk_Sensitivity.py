...
burn_prob_input = st.sidebar.number_input(
    "Current Annual Burn Prob (%)", 
    value=1.0, 
    step=0.01,
    format="%.2f"
)
current_burn_prob = burn_prob_input / 100
...
# --- CALCULATION LOGIC ---
def calculate_metrics(override_burn_prob=None):
    calc_burn_prob = override_burn_prob if override_burn_prob is not None else current_burn_prob

    total_premium = n_homes * avg_premium
    total_uw_expense = total_premium * expense_ratio

    # --- STATUS QUO ---
    sq_losses = n_homes * avg_tiv * calc_burn_prob * mdr_unmitigated
    ...
    faura_loss_unconverted = n_unconverted * avg_tiv * calc_burn_prob * mdr_unmitigated
    faura_loss_converted = n_converted * avg_tiv * calc_burn_prob * mdr_mitigated
...
if tiv_risk_pool > 0 and mdr_diff > 0:
    crossover_burn_prob = total_program_cost / (tiv_risk_pool * mdr_diff)
else:
    crossover_burn_prob = 0 
crossover_percent = crossover_burn_prob * 100
...
for prob in scenarios:
    res = calculate_metrics(override_burn_prob=prob)
...
fig.update_layout(
    title=dict(
        text="Net Profit Comparison by Burn Probability",
        font=dict(size=26)
    ),
    xaxis=dict(
        title="Burn Probability Scenario",
...
with col_right:
    ...
    * **At High Risk (1.00%):** The expected savings jumps to **\${high_weighted_savings:,.0f}**. Since the program cost stays flat at \${avg_prog_cost:,.0f}, **you gain massive profit.**
    ...
    if crossover_percent > 0:
        ...
        st.success(f"✅ **Critical Insight:** For your current inputs, Faura becomes profitable once the annual burn probability exceeds **{crossover_percent:.3f}%**.")