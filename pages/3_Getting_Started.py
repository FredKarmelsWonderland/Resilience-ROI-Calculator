...
def load_data():
    try:
        ...
        # Ensure numeric types for the critical columns used in your baseline
        cols_to_clean = ["TIV", "Annual_Premium", "gross_expected_loss", "scaled_QA_wildfire_score", "carrier_net"]
        for col in cols_to_clean:
            if col in df_scored.columns:
                # Remove currency symbols if present and convert to float
...
# FIX: Use correct column 'gross_expected_loss'
total_gel = df["gross_expected_loss"].sum()
# FIX: Use correct column 'carrier_net'
net_portfolio = df["carrier_net"].sum()
# FIX: Use correct column 'scaled_QA_wildfire_score'
...
with st.expander("📋 View Full Portfolio Metrics", expanded=False):
    # Select only relevant columns for cleanliness
...
with col_hist1:
    # FIX: Use 'scaled_QA_wildfire_score'
...
st.dataframe(top_n[show_cols].style.format({
    "TIV": "${:,.0f}", 
    "Annual_Premium": "${:,.0f}", 
    "gross_expected_loss": "${:,.0f}",
    "scaled_QA_wildfire_score": "{:.1f}"
...
# Simulation Logic using 'gross_expected_loss'
res_faura = {
